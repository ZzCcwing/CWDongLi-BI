...
public class ChartController {
    ...
    @Resource
    public BiMessageProducer biMessageProducer;
    ...
    public BaseResponse<BiResponse> genChartByAi(@RequestPart("file") MultipartFile multipartFile,
        ...
        // 校验文件 首先那道用户请求的文件，取到原始大小的文件
        long size = multipartFile.getSize();
        // 取到原始文件名
        String originalFilename = multipartFile.getOriginalFilename();
        // 校验文件大小  (一兆(1MB) = 1024*1024字节(Byte) = 2的20次方字节)
        final long ONE_MB = 1024 * 1024L;
        // 如果文件大小大于一兆，就抛出异常，并提示文件超过1M
        ThrowUtils.throwIf(size > ONE_MB, ErrorCode.PARAMS_ERROR, "文件过大（超过1M）");
        // 校验文件后缀 （利用FileUtil工具类中的getSuffix方法获取文件后缀名）
        String suffix = FileUtil.getSuffix(originalFilename);
        // 定义合法的后缀列表
        final List<String> validFileSuffixList = Arrays.asList("xlsx", "xls");
        // 如果suffix的后最不在List的范围内，抛出异常，并提示“文件后缀非法”
    ...
    public BaseResponse<BiResponse> genChartByAiAsync(@RequestPart("file") MultipartFile multipartFile,
        ...
        // 在最终的返回结果前提交一个任务
        // todo 建议处理任务队列满了之后，抛异常的情况(因为提交任务报错了,前端会返回异常)
